...
Replace `<your-verified-Blend-address>` with your actual signed-in Blend mailbox.
Use that same verified account in two roles:

- **Project owner:** `<your-verified-Blend-address>` in Codex.
...
Use the three Markdown files in `docs/demo-data/` in this order:

1. `project-brief.md`
2. `architecture.md`
3. `steering-notes.md`

These files were written from scratch for a fictional Demo Harbor service
planning project. Names, architecture, dates, and figures are invented. They
contain no client deck extracts and require no private dataset. Upload only
these three files; this runbook includes the expert answer separately so the
initial document collection retains a deliberate knowledge gap.
...
The bundled Markdown exercises text ingestion and grounded retrieval. To
rehearse visual extraction, separately create a PDF or slide from the same
synthetic architecture table and label that additional evidence accordingly.

### Deliberate synthetic knowledge gap

The steering notes leave the owner and subject-matter expert unspecified for
three fictional planning tools: Arrival Forecast, Supply Alert, and Capacity
Planner. The other two documents do not contain those names either.

Use this exact question:

> Who owns Arrival Forecast, Supply Alert, and Capacity Planner, and who is
> the subject-matter expert for each?

Search should find the explicit gap. The agent should report the missing names,
ask the exact verified mailbox selected for the rehearsal, and index the reply
as demo-only knowledge after review. Keep the synthetic answer in section 10
out of the initial upload set.
...
3. Sign in with the verified Blend360 Microsoft account.
4. Complete the Entra authorization brokered by Cognito.
5. Return to Codex.
6. Ask:
...
- the Microsoft sign-in page, without recording any credentials;
...
1. In Codex, ask:
   ...
   > Add `project-brief.md`, `architecture.md`, and `steering-notes.md` from
   > `docs/demo-data/` to this project and wait until all three are ready to search.
...
9. Repeat the prepared upload flow for all three synthetic files. Each fallback URL
...
1. Ask:
   ...
   > Open the complete extracted text for “architecture.md”. Explain the
   > before/after service-planning table and cite its section heading.
...
- a source-text response with document section locators;
- the before/after table, or a visual description if a rendered fixture was used;
...
1. Ask a broad discovery question:
   ...
   > Which project describes moving Demo Harbor service planning from manual
   > updates to a scheduled import?

2. Verify that the agent uses `search_all_projects`.
3. Ask a project-specific question the synthetic sources can answer:

   > In the Demo Harbor project, how did imports, validation, planning, and
   > review change? Cite each claim to the relevant document section.
...
5. Ask:
   ...
   > Who owns Arrival Forecast, Supply Alert, and Capacity Planner, and who
   > is the subject-matter expert for each?
...
1. Start with a natural prompt:
   ...
   > Create an executive dossier for the fictional Demo Harbor service
   > planning project. Label its figures and outcomes as synthetic.
...
1. Return to the unsupported demo ownership question identified earlier.
2. Ask:

   > Ask `<your-verified-Blend-address>` who owns Arrival Forecast,
   > Supply Alert, and Capacity Planner in the fictional Demo Harbor project,
   > and who the subject-matter expert is for each.
...
6. Write a concrete answer that directly addresses the question.
   ...
   For this isolated rehearsal, use the synthetic answer below:

   ```text
   DEMO-ONLY RESPONSE — this is synthetic ownership data for the Relay
   Workflow Demo project.

   - Arrival Forecast: Avery Example is the demo owner;
     Morgan Example is the accountable demo SME.
   - Supply Alert: Jordan Example is the demo owner;
     Casey Example is the accountable demo SME.
   - Capacity Planner: Taylor Example is the demo owner; Riley
...
8. In a second rehearsal, optionally attach a small supported document
   containing the same demo-only mapping to demonstrate
