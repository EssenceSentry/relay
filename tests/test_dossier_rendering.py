...
def _fixture_markdown(name: str = "synthetic-retail.md") -> str:
...
def test_latex_preserves_content_and_escapes_special_characters() -> None:
    ...
    assert "Demo Orchard Orders \\& Returns" in latex
    assert "\\metric{\\$12.5k}{savings}" in latex
    assert "\\$12.5k savings" in latex
    assert "Synthetic retail scenario" in latex
